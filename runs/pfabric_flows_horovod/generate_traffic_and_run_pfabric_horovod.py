...
def individual_pfabric_run(args):
    ...
    new_run_dir = f"{program_name}_{utilization_interval_ns[0]}_arrival_{config.c_flow_arr_rate}_runhvd_{config.c_run_horovod}_hrvprio_{config.c_horovod_prio}_{curr_date}"
...
class pfabric_horovod_config:
    def __init__(self, link_bw_Mbits, simulation_ns, flow_arr_rate, horovod_prio, run_horovod):
        ...
        if run_horovod == "true":
            for horovod_prio in ["0x10", "0x08"]:
                ...
                # for flows in test_flows:
                    new_config = pfabric_horovod_config(link_bw_Mbits, simulation_ns, flows, horovod_prio, run_horovod)
