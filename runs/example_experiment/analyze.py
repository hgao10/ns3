...
def analyze():
    ...
    for arrival_rate_flows_per_s in arrival_rates:
        for seed in seeds:
            ...
            run_folder_path = "runs/run_s" + str(seed) + "_" + str(arrival_rate_flows_per_s)
    ...
    for stat in ['num_total', 'num_completed', 'num_dnf', 'num_err', 'fct_ns_mean', 'fct_ns_median',
        ...
        with open("data/%s.txt" % stat, "w+") as f_out:
            for arrival_rate in arrival_rates:
                values = []
                ...
                f_out.write("%d\t%f\t%f\t%d\n" % (
                    arrival_rate,
                    1.0 * np.mean(values),
                    2 * np.std(values, ddof=1),  # 2 times the sample standard deviation,
                                                 # which is approximately 95% confidence according to 68-95-99.7 rule
                    len(values)
                ))
