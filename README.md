...
This repository enables the convenient execution of packet-level experiments using the ns-3 simulator. It only takes in the input of a configuration file, which describes the minimal necessary parameters and a topology file. Default routing applied is switch-seeded 5-tuple ECMP.
...
## Example application #1: flow schedule (scratch/main)
...
Flow arrival schedule. This is set by the key `filename_schedule` in `config_ns3.properties`. Each line defines a flow as follows:
...
The pingmesh application is when you want to continuously sends UDP pings between endpoints to measure their RTT. It requires to set `pingmesh_interval_ns` in `config_ns3.properties`.
