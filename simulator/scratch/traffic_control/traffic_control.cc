...
void
TcBytessInQueueTrace (uint32_t oldValue, uint32_t newValue)
{
  std::cout << "TcBytesInQueue " << oldValue << " to " << newValue << std::endl;
}
...
main (int argc, char *argv[])
{
  ...
  std::cout<<"queuedisc at 0: "<<qdiscs.Get(0)->GetTypeId()<<std::endl;
  std::cout<<"queuedisc at 1: "<<qdiscs.Get(1)->GetTypeId()<<std::endl;
  q->TraceConnectWithoutContext ("PacketsInQueue", MakeCallback (&TcPacketsInQueueTrace));
  q->TraceConnectWithoutContext ("BytesInQueue", MakeCallback (&TcBytessInQueueTrace));
